...
class SpeechSynthesisButton extends RectangularPushButton {
  ...
  constructor( property, options ) {
    ...
    const listener = () => {
      voicingManager.cancel(); // cancel any existing speaking

      // read out a number by integer => word or just read out a string
      options.readNumber ? voicingManager.speakImmediately( NumberPlayConstants.NUMBER_TO_STRING[ property.value ] ) :
      voicingManager.speakImmediately( property.value );
    };
